...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, 'chalice') and npcHandler.topic[cid] == 0 then
        npcHandler:say({
            "Finally. That's what I... oh wait, you're still talking to me - you will blow my cover! What do you want? Oh wait, did my {daughter} send",
            "you? It has been some time now, indeed."
        }, cid)
        npcHandler.topic[cid] = 0
    end

 
    if player:getStorageValue(10164) <= 0 then 
        if msgcontains(msg, 'daughter') then
            npcHandler:say({"Well, besides being a renowned sorcerer and scholar at the magic academy in Edron, I am also the proud father of a beautiful daughter, Ivalisse. My little endeavour here must have troubled the poor thing all the time since I left. ...",
	      "Tell her I love her and that I will take care. I have to press on! My research is most valuable to the 'Edron-Zaoan Exploration Division'. You don't happen to have some water, do you? Ah nevermind. It's a long {story}."}, cid)
            npcHandler.topic[cid] = 1	
     
        elseif msgcontains(msg, 'story') and npcHandler.topic[cid] == 1 then
            npcHandler:say({
		...
		"Well, basically Spectulus is the only one likeminded so far. In fact, he incited me to work in the field in first place. So here I am!{disguised} as a chalice."}, cid)
            npcHandler.topic[cid] = 2

        elseif msgcontains(msg, 'disguised') and npcHandler.topic[cid] == 2 then
            npcHandler:say({
		...
		"They also 'used' me a bit, you don't want to go into any details here. I am really {thirsty}."}, cid)
            npcHandler.topic[cid] = 3

        elseif msgcontains(msg, 'thirsty') and npcHandler.topic[cid] == 3 then
            npcHandler:say({
        ...
        elseif msgcontains(msg, 'learned') and npcHandler.topic[cid] == 4 then
            npcHandler:say({
